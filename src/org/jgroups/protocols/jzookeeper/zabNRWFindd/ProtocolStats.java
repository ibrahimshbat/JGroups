...
public class ProtocolStats {
	...
	public AtomicInteger numCommit;
	public AtomicInteger lastNumCommit;
	...
	public ProtocolStats(String protocolName, int numberOfClients,
		...
		this.numCommit = new AtomicInteger(0);
		this.lastNumCommit =  new AtomicInteger(0);
	...
	public void printProtocolStats(int deliveredRequest, int cliuterSize, int perRW, List<Integer> commitDelay){ // For Measuring d, List<Double> commitDelay) {
		int commitArrivalRate = 0;
		double comDelay = 0.0, avgAllD = 0, avgRead=0, allRWAvg=0, throughputRate =0, tempSumThr=0, per50th=0, per90th=0, 
				per99th=0, per95th=0;
		String CI95=null;
		...
		//For Measuring d
//		for (double del:commitDelay){
//			comDelay+=del;
//		}
//		comDelay= (double) comDelay/commitDelay.size();
//		outFile.println("Commit Delay: "+comDelay);
		
		//For Measuring commit Arrival Rate
		for (double del:commitDelay){
			commitArrivalRate+=del;
		}
		commitArrivalRate=  commitArrivalRate/commitDelay.size();
		outFile.println("Commit Arrival Rate/sec: "+commitArrivalRate);
		outFile.println("/Num of Commit overall=:"+numCommit.get());
