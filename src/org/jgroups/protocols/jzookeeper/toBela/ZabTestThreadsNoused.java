...
/*
 * This class is responsible for generating and managing senders who send request to Zab protocol.
 */
public class ZabTestThreadsNoused extends ReceiverAdapter {
	...
	private ZabClientNoUsed[] clientThreads;
	...
	private static int ZabClusterSize = 3;
	...
	static {
		try {
			METHODS[SETUPCLIENTS] = ZabTestThreadsNoused.class
					.getMethod("setupClientThreads");//Creating senders and its channel
			METHODS[RESET] = Zab.class
					.getMethod("reset");//Reset Zk servers after warmup phase
			METHODS[STARTREALTEST] = ZabTestThreadsNoused.class
					.getMethod("startRealTest");// Sender starts the real test (The workload)
	...
	public ZabTestThreadsNoused() {
	}

	public ZabTestThreadsNoused(String[] zabHosts, String protocolName, String props,
	...
	/* 
	 * This method creates a channel for the controller and RpcDispatcher
	 */
	...
	/* 
	 *  This methods is invoked by initiator who sends RPC to all controller (one by one) 
	 *  telling them to create their senders
	 */
	...
	/*
	 * This method is invoked by RPC which tells this controller
	 *  to create and setup its senders
	 */
	public void setupClientThreads() {

		final CyclicBarrier barrier = new CyclicBarrier(num_threads + 1);
		clientThreads = new ZabClientNoUsed[num_threads];
		for (int i = 0; i < clientThreads.length; i++) {
			clientThreads[i] = new ZabClientNoUsed(zabBox, barrier, num_msgs,
     ...
     /*
      * This method starts the warm up phase to warm up the Zab protocol
      *  before the real workload starting
      */
	public void warmup() {
		for (int i = 0; i < clientThreads.length; i++) {
	...
	/*
	 * This method tells the Zab to reset its state before the real test begin
	 */
	...
	/*
	 * This method starts real test (workload). It invokes by RPC.
	 */
	...
	/*
	 * This method checks if all senders that associate with this controller finish
	 * its workload.
	 *  
	 */
	...
	/*
	 * This method sends requests to ZK server (follower and leader) to print their
	 *  profiler (latency and throughput) that was collected during the workload  
	 */
	...
	public static void main(String[] args) {
		...
		System.out.println("propsFile " + propsFile + "/" + "zabboxInits "
				...
				+ "numWarmUp " + numWarmUp + "/" + "sendTime " + sendTime + "/" + "Initiator " + initiator);
		try {

			final ZabTestThreadsNoused test = new ZabTestThreadsNoused(zabboxInits, name,
					propsFile, totalMessages, numberOfMessages, numsThreads,
					msgSize, outputDir, numOfClients, load, numWarmUp, sendTime, initiator);
