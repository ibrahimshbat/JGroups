...
public class ZabTestThreads extends ReceiverAdapter {
	...
	static {
		try {
			METHODS[SETUPCLIENTS] = ZabTestThreads.class
					.getMethod("setupClientThreads");// Creating senders and its
														// channel
			METHODS[RESET] = Zab.class.getMethod("reset");// Reset Zk servers
															// after warmup
															// phase
			METHODS[STARTREALTEST] = ZabTestThreads.class
					.getMethod("startRealTest"); // Sender starts the real test
													// (The workload)
	...
	/*
	 * This methods is invoked by initiator of the test and sends RPC to all
	 * controller (one by one) telling them to create their senders
	 */
	...
	/*
	 * This method is invoked by RPC which tells this controller to create and
	 * setup its senders
	 */
	...
	/*
	 * This method starts the warm up phase to warm up the Zab protocol before
	 * the real workload starting
	 */
	public void warmup() {
		for (int i = 0; i < clientThreads.length; i++) {
	...
	/*
	 * This method checks if all senders that associate with this controller
	 * finish its workload.
	 */
	...
	/*
	 * This method sends requests to ZK server (follower and leader) to print
	 * their profiler (latency and throughput) that was collected during the
	 * workload
	 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			final ZabTestThreads test = new ZabTestThreads(zabboxInits, name,
					...
					msgSize, outputDir, numOfClients, load, numWarmUp,
					sendTime, initiator);
