 ...
 */
public class ZabClientNoUsed extends ReceiverAdapter {
    ...
    private ZabTestThreadsNoused zabTest= new ZabTestThreadsNoused();


	public ZabClientNoUsed(List<Address> zabbox, CyclicBarrier barrier, long numsMsg, AtomicLong local,
			byte[] payload, String ProtocotName, long num_msgsPerThreads, String propsFile, int load, long warmUpRequests, int sendTime, ZabTestThreadsNoused zabTest ) {
	...
	/* 
	 * This method creates a sender and tells it to start  sending
	 *  a request to the ZK servers
	 */
	...
	public void receive(Message msg) {
		synchronized (this) {
			...
			if (is_warmUp){// Response for For warm
				...
				else if(testHeader.getType()==ZabHeader.RESPONSE){ //Response (write request) for real test 
					msgReceived++;
	...
	public class Sender extends Thread {
		 ...
		 * Note that there are two way to send a request. One way is using
		 * sync mode which means the sender send next request as soon as receive
		 * replay for the current request. Second way sender sends a request
		 * and wait for Delta time and then as soon as Delta elapse, it send the next request. 
		 */
		public void run() {
...
//					sendTime = min + rand.nextInt((max - min) + 1); // For stagged the senders 
