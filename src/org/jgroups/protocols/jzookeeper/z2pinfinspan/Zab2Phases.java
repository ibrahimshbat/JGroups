...
//				break;
	...
	private synchronized void sendACK(Message msg, Zab2PhasesHeader hdrAck) {
		...
		//log.info("hdrAck.getMessageOrderInfo():= "+hdrAck.getMessageOrderInfo());
		queuedProposalMessage.put(messageOrderInfo.getOrdering(), hdrAck);
		//if(messageOrderInfo.getOrdering()==1){
			//log.info("hdrAck.getMessageOrderInfo().getOrdering():= "+hdrAck.getMessageOrderInfo().getOrdering());
			//log.info("queuedProposalMessage.get:= "+queuedProposalMessage.get(messageOrderInfo.getOrdering()));			
		//}
	...
	private void sendOrderResponse(MessageOrderInfo messageOrderInfo){
		...
		Message msgResponse = new Message(messageOrderInfo.getId()
				.getOriginator()).putHeader(ClassConfigurator.getProtocolId(CSInteraction.class), hdrResponse);
