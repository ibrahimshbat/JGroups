...
public class Zab2PhasesBundling extends Protocol {
	...
	private final Map<Long, MessageOrderInfo> queuedProposalMessage = Collections
			.synchronizedMap(new HashMap<Long, MessageOrderInfo>());
	...
	public Object up(Event evt) {
		...
		case Event.MSG:
			...
			case Zab2PhasesHeader.PROPOSAL:
				...
				sendACK(hdr.getMessageOrderInfo());
				break;
			case Zab2PhasesHeader.BUNDLEPROPOSAL:
				receiveMultipleOrderings(hdr, msg);
				break;
	...
	public void receiveMultipleOrderings(Zab2PhasesHeader header, Message message) {
		Collection<MessageOrderInfo> orderings = header.getBundledMsgInfo();
		if (log.isTraceEnabled())
			log.trace("Received multiple orderings (" + orderings.size() + ")| " + orderings);

		for (MessageOrderInfo info : orderings)
			sendACK(info);
	 ...
	 */
	private synchronized void sendACK(MessageOrderInfo messageOrderInfo) {
		//if (msg == null)
		//return;
		...
		queuedProposalMessage.put(messageOrderInfo.getOrdering(), messageOrderInfo);
	...
	private void deliver(long dZxid) {
		...
		messageOrderInfo = queuedProposalMessage.remove(dZxid);
		if(messageOrderInfo==null){
		...
		queuedCommitMessage.put(dZxid, messageOrderInfo);
	...
	private void sendOrderResponse(MessageOrderInfo messageOrderInfo){
		...
		Message msgResponse = new Message(messageOrderInfo.getId()
				.getOriginator()).putHeader(ClassConfigurator.getProtocolId(CSInteraction.class), hdrResponse);//.setFlag(Message.Flag.DONT_BUNDLE);;
	...
	final class FollowerMessageHandler implements Runnable {
		...
		private void handleSingleRequest(Zab2PhasesHeader header){
			...
			storeProposalLocal(messageOrderInfo);
		...
		private void handleMultiRequests(List<MessageOrderInfo> messageOrderInfos){
			Zab2PhasesHeader bundleProposals = new Zab2PhasesHeader(Zab2PhasesHeader.BUNDLEPROPOSAL, messageOrderInfos);
			Message bundleMessage = new Message().putHeader(this.id,
					bundleProposals).setFlag(Message.Flag.DONT_BUNDLE);
			...
			try {
				...
				for (Address address : zabMembers) {
					if (address.equals(leader))
						continue;
					Message cpy = bundleMessage.copy();
					cpy.setDest(address);
					down_prot.down(new Event(Event.MSG, cpy));
				}
			} catch (Exception ex) {
			...
			while (running) {
				...
				if (BUNDLE_MSGS) {
					...
					if (queuedMessages.isEmpty()) {
						handleSingleRequest(hdrReq);
					} else {
						List<MessageOrderInfo> requests = new ArrayList<MessageOrderInfo>();
						new_zxid = getNewZxid();
						messageOrderInfo = hdrReq.getMessageOrderInfo();
						messageOrderInfo.setOrdering(new_zxid);
						storeProposalLocal(messageOrderInfo);
						requests.add(messageOrderInfo);
						while ((hdrReq = queuedMessages.peek()) != null && (bundleSize + hdrReq.size()) < MSG_SIZE) {
							queuedMessages.poll(); // Just removes header. Peek necessary because of the size check
							bundleSize += hdrReq.size();
							new_zxid = getNewZxid();
							messageOrderInfo = hdrReq.getMessageOrderInfo();
							messageOrderInfo.setOrdering(new_zxid);
							storeProposalLocal(messageOrderInfo);
							requests.add(messageOrderInfo);
							//log.info("bundle size=: "+bundleSize+ " bundleReqn=: "+requests.size());
						}
						handleMultiRequests(requests);
					}
				} else {
					handleSingleRequest(hdrReq);
				}
		...
		private void storeProposalLocal(MessageOrderInfo messageOrderInfo){

			Proposal p = new Proposal();
			p.setMessageOrderInfo(messageOrderInfo);
			p.AckCount++;
			outstandingProposals.put(messageOrderInfo.getOrdering(), p);
			queuedProposalMessage.put(messageOrderInfo.getOrdering(), messageOrderInfo);
