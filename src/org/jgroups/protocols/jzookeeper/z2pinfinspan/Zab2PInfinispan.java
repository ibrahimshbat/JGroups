 ...
 *
 */
public class Zab2PInfinispan extends ReceiverAdapter {
	...
	static {
		try {
			METHODS[START]                 = Zab2PInfinispan.class.getMethod("startTest");
			METHODS[SET_OOB]               = Zab2PInfinispan.class.getMethod("setOOB", boolean.class);
			METHODS[SET_SYNC]              = Zab2PInfinispan.class.getMethod("setSync", boolean.class);
			METHODS[SET_NUM_MSGS]          = Zab2PInfinispan.class.getMethod("setNumMessages", int.class);
			METHODS[SET_NUM_THREADS]       = Zab2PInfinispan.class.getMethod("setNumThreads", int.class);
			METHODS[SET_MSG_SIZE]          = Zab2PInfinispan.class.getMethod("setMessageSize", int.class);
			METHODS[SET_ANYCAST_COUNT]     = Zab2PInfinispan.class.getMethod("setAnycastCount", int.class);
			METHODS[SET_USE_ANYCAST_ADDRS] = Zab2PInfinispan.class.getMethod("setUseAnycastAddrs", boolean.class);
			METHODS[SET_READ_PERCENTAGE]   = Zab2PInfinispan.class.getMethod("setReadPercentage", double.class);
			METHODS[GET]                   = Zab2PInfinispan.class.getMethod("get", long.class);
			METHODS[PUT]                   = Zab2PInfinispan.class.getMethod("put", long.class, byte[].class);
			METHODS[GET_CONFIG]            = Zab2PInfinispan.class.getMethod("getConfig");
	...
	public void init(List<String> Members, String protocolName, String props,
			...
			channelName, String initiator, int rf) throws Throwable {
	...
	public void viewAccepted(View new_view) {
		...
		if (mbrs.size() > clusterSize && box.isEmpty()) {
	...
	public Results startTest() throws Throwable {
		...
		for(int i=0; i < invokers.length; i++){
			invokers[i]=new Invoker(nonBoxMembers, (num_msgsPerThreads * num_threads), num_msgs_sent, random);
	...
	void startBenchmark() throws Throwable {
		...
		System.out.println("\n======================= Results:===========================");
		System.out.println("========= Replication Factor( "+(anycast_count+1)+" ):=========");

		outFile.println("\n============================Results:===============================");
		outFile.println("============== Replication Factor( "+(anycast_count+1)+" ):============");
	...
	private class Invoker extends Thread {
		...
		public void run() {
			...
			while(true) {
				...
				System.out.println("num_msgs_sent"+num_msgs_sent.get());
				...
				try {
					if(get) { // sync GET
						...
						//System.out.println("Elapsed rpc "+(System.currentTimeMillis()-start));
						//rsp.
						num_puts++;
	...
	public static void main(String[] args) {
		...
		Zab2PInfinispan test=null;
		test=new Zab2PInfinispan();
		try {
