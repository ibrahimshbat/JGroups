...
public class Zab2Phases extends Protocol {
	...
	private synchronized void handleClientRequest(Message message) {
		...
		if (clientHeader != null) {
			...
			Zab2PhasesHeader zab2PHeader = new Zab2PhasesHeader(Zab2PhasesHeader.REQUEST, msgId);
			forwardToLeader(message, zab2PHeader);
	 ...
	 */
	private void forwardToLeader(Message msg, Zab2PhasesHeader hdrCT) {
	 ...
	 */
	private void forward(Zab2PhasesHeader hdr2P) {
		try {
			Zab2PhasesHeader hdr = new Zab2PhasesHeader(Zab2PhasesHeader.FORWARD, hdr2P.getMessageId());
	...
	private void deliver(long dZxid) {
		...
		if (messageStore.containsKey(hdrOrginal.getMessageId())) {
			...
			Zab2PhasesHeader hdrResponse = new Zab2PhasesHeader(Zab2PhasesHeader.RESPONSE, dZxid, hdrOrginal.getMessageId());
