...
import org.jgroups.protocols.jzookeeper.ZabClients.Sender;
...
public class ZabTestThreadss extends ReceiverAdapter {
	...
	private ZabClients[] clientThreads;
	...
	private static final short STARTREALTS = 3;

	static {
		try {
			METHODS[SETUPCLIENTS] = ZabTestThreadss.class
					.getMethod("setupClientThreads");
			METHODS[RESET] = Zab2Phases.class
					.getMethod("reset");
			METHODS[STARTREALTS] = ZabTestThreadss.class
					.getMethod("startRealT");
	...
	public ZabTestThreadss(String [] zabHosts, String protocolName, String props,
	...
	// Invoke by Initiator
			...
			for (Address client : clients) {
				try {
					...
					long time2 =System.currentTimeMillis();   
				     System.out.println("Elapsed rpc time="+(time2-time1));
		...
		public void setupClientThreads() {
			...
			clientThreads = new ZabClients[num_threads];
			// if (!zabboxInit.contains(local_addr.toString().split("-")[0])) {
				for (int i = 0; i < clientThreads.length; i++) {
					clientThreads[i] = new ZabClients(zabBox, barrier, num_msgs,
...
//	}
 ...
 public void resetProtocol() throws Exception {
		...
		//for (Address zabNode : zabBox) {
			//try {
				//System.out.println("RPC RESET TO "+zabNode);
				//long time1 =System.currentTimeMillis();
//				 RequestOptions options=new RequestOptions(ResponseMode.GET_ALL, 40000);
//			     options.setFlags(Message.Flag.OOB, Message.Flag.DONT_BUNDLE, Message.NO_FC);
//			     RspList<Object> responses=disp.callRemoteMethod(zabNode, new MethodCall(RESET), options);
				//long time2 =System.currentTimeMillis();   
			     //System.out.println("Elapsed rpc time="+(time2-time1));
			     //System.out.println("after sending rpc");
				//.sleep(40000);
		...
		for (Address client : clients) {
			try {
				...
				//long time1 =System.currentTimeMillis();
				 RequestOptions options=new RequestOptions(ResponseMode.GET_ALL, 4000);
			     options.setFlags(Message.Flag.OOB, Message.Flag.DONT_BUNDLE, Message.NO_FC);
			     RspList<Object> responses=disp.callRemoteMethod(client, new MethodCall(STARTREALTS), options);
				//long time2 =System.currentTimeMillis();   
			     //System.out.println("Elapsed rpc time="+(time2-time1));
			     //System.out.println("after sending rpc");
				//.sleep(40000);
...
//	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			final ZabTestThreadss test = new ZabTestThreadss(zabboxInits, name, propsFile, totalMessages,
	 ...
	 static class CustomMarshaller implements RpcDispatcher.Marshaller {
	        ...
	        public Buffer objectToBuffer(Object obj) throws Exception {
	            ...
	            switch(call.getId()) {
	                ...
	                case STARTREALTS:
		             	   buf=ByteBuffer.allocate(Global.BYTE_SIZE);
                        System.out.println("Get id for STARTREALTS="+(byte)call.getId());                                         
		                    buf.put((byte)call.getId());
		                    return new Buffer(buf.array());
	                default:
