package org.jgroups.protocols.jzookeeper.bundle;
...
public class ZabBundle extends Protocol {
...
//				log.info("Send STARTREALTEST to "+c);
...
//		}
	...
	public Object down(Event evt) {
		...
		case Event.MSG:
			...
			ZabHeader zh=(ZabHeader) m.getHeader((short)75);
			//log.info("hdr"+zh);
			if (zh instanceof ZabHeader){
				//log.info("hdrInsideIF= "+zh);
				handleClientRequest(m);
				return null;
			}
			else{
				//log.info("hdrInsideELSE"+zh);
				break;
			}
...
//				}
	...
	final class FollowerMessageHandler implements Runnable {
		...
		public void handleMultiRequests(List<ZabHeader> headers){
			...
			while (running) {
				...
				if (BUNDLE_MSGS) {
                    ...
                    if (queuedMessages.isEmpty()) {
                        ...
                        List<ZabHeader> requests = new ArrayList<ZabHeader>();
                        requests.add(hdrReq);
