...
public class Zab extends Protocol {
	...
	private ConcurrentMap<Long, Proposal> queuedCommitMessage = new ConcurrentHashMap<Long, Proposal>();
	private ConcurrentMap<Long, Proposal> queuedDeliveredMessage = new ConcurrentHashMap<Long, Proposal>();
	...
	public void reset() {
		...
		//	throTimer.schedule(new Throughput(), 1000, 1000);
			this.stats = new ProtocolStats(ProtocolName, clients.size(),
					numberOfSenderInEachClient, outDir, 0,false, true);
		}
		log.info("Reset done");
		throTimer.schedule(new ServerThroughput(), 1000, 1000);
		this.serverStats = new ServerStats(ProtocolName, clients.size(),
				numberOfSenderInEachClient, outDir, 50, false, true);
		serverStats.setStartThroughputTime(System.currentTimeMillis());
//		MessageId messageId = new MessageId(local_addr,
//				-10, System.currentTimeMillis());
//		MessageInfo msgInfo = new MessageInfo(messageId);
//		log.info("Clients: "+ clients);
//		for(Address client: clients){
//			log.info("Send to client: "+ client);
//			ZabHeader resetHdr = new ZabHeader(ZabHeader.CLIENTFINISHEDRESET, msgInfo);
//			Message resetMessage = new Message(client).putHeader(this.id, resetHdr);
//			resetMessage.setFlag(Message.Flag.DONT_BUNDLE);
//			resetMessage.setFlag(Message.Flag.OOB);
...
//		}
	...
	public Object up(Event evt) {
        ...
        log.info("Read percentsge====> "+hdr.getMessageInfo().getZxid());
				reset();
				break;
//			case ZabHeader.CLIENTFINISHEDRESET:
//				log.info("Yes, I recieved CLIENTFINISHEDRESET");
//				return up_prot.up(new Event(Event.MSG, msg));		
			...
			case ZabHeader.CLIENTFINISHED:
					...
					if(is_leader)
						running = false;
					if (clientFinished == clients.size()) {
						log.info("Finished In Zab Not Yet printProtocolStats");
						serverStats.setEndThroughputTime(System.currentTimeMillis());
						serverStats.printProtocolStats();
						if (stats.isEnabled){
							throTimer.cancel();
	...
	private synchronized void handleClientRequest(Message message) {
		...
		if (clientHeader != null && clientHeader.getType() == ZabHeader.RESET) {
			...
			if(clientHeader.getType() == ZabHeader.REQUESTW){
				...
				requestMessage.setFlag(Message.Flag.OOB);
			}
	...
	private void deliver(long dZxid) {
		...
		queuedDeliveredMessage.put(dZxid, proposal);
		serverStats.incnumReqDelivered();
		if (stats.isEnabled){
			if (!stats.isWarmup()) {
		...
		log.info("Deliver Zxid: " + dZxid + " #Requested: "+serverStats.getnumReqDelivered());
	...
	private synchronized void readData(MessageInfo messageInfo){
		...
		Proposal proposal = queuedDeliveredMessage.get(messageInfo.getZxid());
		if (proposal != null){
			readReplayHdr = new ZabHeader(ZabHeader.RESPONSER, new MessageInfo(messageInfo.getId(), proposal.getZxid()));
			//log.info("Read Mid: "+messageInfo.getId());
			...
			log.info("Read Mid: proposal is null not found");
	...
	final class ServerThroughput extends TimerTask {

		public ServerThroughput() {
		...
		@Override
		public void run() {
			startTime = serverStats.getLastThroughputTime();
			currentTime = System.currentTimeMillis();
			finishedThroughput = serverStats.getnumReqDelivered();
			currentThroughput = (((double) finishedThroughput - serverStats
					.getLastNumReqDeliveredBefore()) / ((double) (currentTime - startTime) / 1000.0));
			serverStats.setLastNumReqDeliveredBefore(finishedThroughput);
			serverStats.setLastThroughputTime(currentTime);
			serverStats.addThroughput(currentThroughput);
		}
