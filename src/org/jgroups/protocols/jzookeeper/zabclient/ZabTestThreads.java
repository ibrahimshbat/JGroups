...
public class ZabTestThreads extends ReceiverAdapter {
	...
	private String channelName = "Zab";
	...
	private int sTime = 0;
 	...
 	private static int numNodesFinisedReset = 0;
	...
	public void setupClientThreads() {
		...
		for (int i = 0; i < clientThreads.length; i++) {
			clientThreads[i] = new ZabClient(zabBox, barrier, num_msgs,
					...
					propsFile, load, numsOfWarmUpPerThread, sTime, read_percentage ,
					msgReciever, channelName, this);
		}
	...
	public void resetProtocol() throws Exception {
		...
		MessageId mid = new MessageId(local_addr,
				localSequence.incrementAndGet());
		MessageInfo messageInfo = new MessageInfo(mid);
    messageInfo.setZxid((long) (read_percentage*100));
...
//		ZabHeader hdr = (ZabHeader) msg.getHeader(ID);
//		System.out.println("Received CLIENTFINISHEDRESET numNodesFinisedReset= "+numNodesFinisedReset);
//
//		synchronized(this){
//			//ZabHeader hdr = (ZabHeader) msg.getHeader(ID);
//			if (hdr.getType() == ZabHeader.CLIENTFINISHEDRESET) {
//				numNodesFinisedReset++;
//				System.out.println("Received CLIENTFINISHEDRESET numNodesFinisedReset= "+numNodesFinisedReset);
//				if (numNodesFinisedReset == clusterSize)
//				    startRealTest();
//			}
//		}
//	}
	...
	public void startRealTest() {
		...
		this.stats = new ProtocolStats(ProtocotName, numOfClients,
				num_threads, outputDir, read_percentage, false, true);
		for (int i = 0; i < clientThreads.length; i++) {
			clientThreads[i].init();
		}
	...
	public void collectLatencies(){
		for (int i = 0; i < clientThreads.length; i++) {
			for (long latency:clientThreads[i].getReadLatencies())
				stats.addReadLatency(latency);

			for (long latency:clientThreads[i].getWriteLatencies())
	...
	public void printStats(){
		stats.setnumReqDelivered(getMaxNumDelivered());
		System.out.println("#Deleiverd Message: "+msgReciever.get());

		stats.setNumRequest((int) (num_msgsPerThreads*num_threads));
		stats.printProtocolStats();
	}
	
	public int getMaxNumDelivered(){
		long max = Long.MIN_VALUE;
		for (int i = 0;i<clientThreads.length;i++){ 
			if(max < clientThreads[i].getMaxDelivered())
				max = clientThreads[i].getMaxDelivered();
		}
		return (int) max;
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			final ZabTestThreads test = new ZabTestThreads(zabboxInits, name,
					...
					read_percentage, initiator, channelName);
    ...
    class Throughput extends TimerTask {
		...
		public void run() {
			...
			finishedThroughput = getMaxNumDelivered();
