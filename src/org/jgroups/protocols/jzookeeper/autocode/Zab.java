...
/*
 * It is  Zookeeper Atomic Broadcast. Also it has features of testing throuhput, latency, and etc. 
...
public class Zab extends Protocol {
	...
	private final Map<Long, ZabHeader> queuedProposalMessage = Collections
			.synchronizedMap(new HashMap<Long, ZabHeader>()); // Emulate the
																// log, logging
																// the Proposal
	...
	private synchronized void handleClientRequest(Message message) {
		...
		if (clientHeader != null && clientHeader.getType() == ZabHeader.RESET) {
			...
			for (Address server : zabMembers) {
				...
				resetMessage.setFlag(Message.DONT_BUNDLE);
			...
			for (Address server : zabMembers) {
				Message statsMessage = new Message(server).putHeader(this.id,
						clientHeader);
				statsMessage.setFlag(Message.DONT_BUNDLE);
			...
			for (Address server : zabMembers) {
				Message countMessages = new Message(server).putHeader(this.id,
						clientHeader);
				countMessages.setFlag(Message.DONT_BUNDLE);
		    ...
		    requestMessage.setFlag(Message.DONT_BUNDLE);
	...
	private void forward(Message msg) {
		...
		try {
			...
			forward_msg.setFlag(Message.DONT_BUNDLE);
	...
	private void sendACK(Message msg, ZabHeader hdrAck) {
		...
		if (hdrAck == null)
			return;
		queuedProposalMessage.put(hdrAck.getZxid(), hdrAck); // Emulate the log,
																// logging the
																// Proposal,
																// instead will
																// use disk in
		...
		ackMessage.setFlag(Message.DONT_BUNDLE);
		if (!stats.isWarmup()) {
	...
	private void commit(long zxidd) {
		if (zxidd != lastZxidCommitted + 1) {
			if (log.isDebugEnabled()) {
				log.debug("delivering Zxid out of order " + zxidd
		...
		commitMessage.setFlag(Message.DONT_BUNDLE);
	...
	private void deliver(long dZxid) {
		...
		if (requestQueue.contains(hdrOrginal.getMessageId())) {
			...
			msgResponse.setFlag(Message.DONT_BUNDLE);
	...
	private synchronized void addTotalABMssages(
		...
		if (numABRecieved == zabMembers.size() - 1) {
			...
			for (Address zabServer : zabMembers) {
				...
				messageStats.setFlag(Message.DONT_BUNDLE);
		...
		public void handleRequests() {
			...
			while (running) {
				...
				Message ProposalMessage = new Message().putHeader(this.id,
						hdrProposal);
				ProposalMessage.setFlag(Message.DONT_BUNDLE);
