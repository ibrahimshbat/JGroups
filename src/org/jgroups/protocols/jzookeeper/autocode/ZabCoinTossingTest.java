...
//		avgRecievedOps = 0;
//		is_warmUp = false;
//		this.outFile = new PrintWriter(new BufferedWriter(
//				new FileWriter(outputDir + InetAddress.getLocalHost().getHostName() + ".log", true)));
//		MessageId mid = new MessageId(local_addr, localSequence.incrementAndGet());
//		ZabCoinTossingHeader resetProtocol = new ZabCoinTossingHeader(ZabCoinTossingHeader.RESET, -4, mid);
//		Message msg = new Message(null).putHeader(ID, resetProtocol);
//		channel.send(msg);
//	}
	...
	public void sendTestRequest() throws Exception {
		...
		for (int i = 0; i < 1000; i++) {
			...
			MessageId mid = new MessageId(local_addr, localSequence.incrementAndGet());
			// ZabCoinTossingHeader startHeader = new
			// ZabCoinTossingHeader(ZabCoinTossingHeader.TEMPSENT,i, mid);
			// Message msg = new Message(null).putHeader(ID, startHeader);
			// channel.send(msg);
		}
	...
	public synchronized void finishedTest() {
		...
		// outFile.println();
		// outFile.println();
		// outFile.println("Test "+ProtocotName + " /Load " + load + " /numMsgs
		// For Each Thread " + num_msgsPerThreads+
		// " /numMsgs For Each Client " + num_msgs+ " /numsThreads
		// "+num_threads+" /msg size " + msg_size+
		// " /numOfClients All Cluster " + numOfClients+" /numMsgs For All
		// Cluster "+ num_msgs*numOfClients);
		...
		// numOpsRecieved + " Throughput per sender
		// "+(numOpsRecieved/TimeUnit.MILLISECONDS.toSeconds(timeElapsed))+"
		// ops/sec"
		...
		// +(avgRecievedOps/TimeUnit.MILLISECONDS.toSeconds(avgTimeElpased))+"
		// ops/sec");
	...
	public static void main(String[] args) {
		...
		System.out.println("propsFile " + propsFile + "/" + "zabboxInits " + zabboxInits + "/" + "name " + name + "/"
				...
				+ numOfClients + "/" + "load " + load + "/" + "numWarmUp " + numWarmUp+"/"+"sendTime "+sendTime);
		try {

			final ZabCoinTossingTest test = new ZabCoinTossingTest(zabboxInits, name, propsFile, totalMessages,
					numberOfMessages, numsThreads, msgSize, outputDir, numOfClients, load, numWarmUp, sendTime);
